...
pub(crate) fn rocm_memory_analysis_pass(
...
mod tests {
    use super::{
        rocm_memory_analysis_pass, estimate_graph_memory_bytes, loop_marker_keys_for_node,
    };
    use luminal::{
        egglog_utils::{
    ...
    fn cuda_memory_state_split_uses_dynamic_dimensions() {
        ...
        dyn_map.insert('s', 4);
        let late_pass = rocm_memory_analysis_pass(&ops, Some(16), &dyn_map);
        let program = format!(
            r#"
            (let t0 (Input 0 "" (F32)))
            (let t1 (Input 1 "" (F32)))
