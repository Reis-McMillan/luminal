...
pub(crate) struct CompiledBucket {
    ...
    pub(crate) arena: Option<HipSlice<u8>>,
...
pub struct RocmRuntime {
    ...
    /// Non-owning HipSlice wrappers for external device pointers.
    /// ManuallyDrop prevents cuMemFree — the external allocator (e.g. PyTorch) owns the memory.
    external_buffers: FxHashMap<NodeIndex, std::mem::ManuallyDrop<HipSlice<u8>>>,
    ...
    /// Non-owning HipSlice views of external output pointers, keyed by LLIR data node
    /// ManuallyDrop prevents cuMemFree -- Pytorch owns the memory
    external_output_buffers: FxHashMap<NodeIndex, std::mem::ManuallyDrop<HipSlice<u8>>>,
}
...
impl RocmRuntime {
    ...
    fn resolve_runtime_buffer(
        bucket: &CompiledBucket,
        stream: &Arc<HipStream>,
        hlir_buffers: &FxHashMap<NodeIndex, RocmInput>,
        external_buffers: &FxHashMap<NodeIndex, std::mem::ManuallyDrop<HipSlice<u8>>>,
        external_output_buffers: &FxHashMap<NodeIndex, std::mem::ManuallyDrop<HipSlice<u8>>>,
        mut node: NodeIndex,
    ) -> Option<DeviceBuffer> {
    ...
    pub fn set_zeros(&mut self, id: impl ToId, num_bytes: usize) {
        let id = id.to_id();
        let buf = self.hip_stream.alloc_zeros(num_bytes).unwrap();
        self.hlir_buffers.insert(id, RocmInput::Buffer(buf));
        self.changed_hlir.insert(id);
    }
    ...
    pub unsafe fn set_device_ptr(&mut self, id: impl ToId, device_ptr: u64, n_bytes: usize) {
        ...
        self.hlir_buffers.insert(id, RocmInput::Ptr(device_ptr));
        self.changed_hlir.insert(id);
    }
    ...
    fn get_output_data(&self, id: impl ToId) -> Vec<u8> {
        ...
        if let Some(hlir_node) = bucket.llir_to_hlir.get(&data_id) {
            ...
            {
                RocmInput::Buffer(buf) => self.hip_stream.clone_dtoh(buf).unwrap(),
                RocmInput::Ptr(_) => {
                    // External device pointer — use the HipSlice view from external_buffers
                    if let Some(ext) = self.external_buffers.get(hlir_node) {
                        self.hip_stream.clone_dtoh(&**ext).unwrap()
                    } else {
                        panic!(
            ...
            if let Some(ext) = self.external_output_buffers.get(&data_id) {
                return truncate_to_logical_bytes(self.hip_stream.clone_dtoh(&**ext).unwrap());
            }

            // Predecessor is a computation node — data is in the intermediate arena.
            truncate_to_logical_bytes(
                Self::bucket_buffer(bucket, &self.hip_stream, &data_id)
                    .expect("Cannot find tensor in runtime!")
                    .clone_dtoh(&self.hip_stream)
                    .unwrap(),
            )
        }
    }
    ...
    pub fn remove_buffer(&mut self, id: impl ToId) -> HipSlice<u8> {
        let producer = self.find_producer_node(id);
        ...
        if alias_node == lineage_node {
            ...
            if let Some(hlir_node) = hlir_node {
                match self
                    ...
                    .expect("Cannot find input tensor in runtime!")
                {
                    RocmInput::Buffer(buf) => buf,
                    RocmInput::Ptr(p) => panic!("Cannot take raw pointer input (ptr=0x{:x})", p),
                }
            } else {
                ...
                .expect("Cannot find tensor in runtime!");
                Self::copy_device_buffer_to_new_slice(&self.hip_stream, src)
            }
        } else {
            ...
            let output =
                Self::bucket_buffer(&self.compiled_buckets[bi], &self.hip_stream, &alias_node)
                    .expect("Cannot find intermediate output buffer in runtime!");
            let output_buf = Self::copy_device_buffer_to_new_slice(&self.hip_stream, output);
    ...
    pub fn set_buffer(&mut self, id: impl ToId, buf: HipSlice<u8>) {
        let id = id.to_id();
        self.hlir_buffers.insert(id, RocmInput::Buffer(buf));
        self.changed_hlir.insert(id);
    }
    ...
    pub fn swap_output_to_input(&mut self, output_id: impl ToId, input_id: impl ToId) {
        ...
        let input_buf = Self::copy_device_buffer_to_new_slice(&self.hip_stream, src);
        self.hlir_buffers
            .insert(input_id, RocmInput::Buffer(input_buf));
    ...
    /// ROCm graph building is handled internally by RocmGraphOp on first execution.
    #[tracing::instrument(skip_all)]
    pub fn prebuild_graphs(&mut self, dyn_map: &FxHashMap<char, usize>) {
        ...
        Self::allocate_intermediate_buffers(bucket, &self.hip_stream, dyn_map);
...
impl Runtime for RocmRuntime {
    type Ops = (crate::kernel::Ops, crate::host::Ops);
    type CompileArg = Arc<HipStream>;
    type ExecReturn = ();
    ...
    fn allocate_dummy_input(&mut self, node_index: usize, num_bytes: usize) {
        ...
        let buf = self.hip_stream.clone_htod(&host_data).unwrap();
    ...
    fn has_nan_outputs(&self, _llir_graph: &LLIRGraph, _dyn_map: &FxHashMap<char, usize>) -> bool {
        ...
        for producer in bucket.output_producers.values().copied() {
            ...
            let Some(buf) = Self::bucket_buffer(bucket, &self.hip_stream, &node_id) else {
                continue;
            };
            let n_bytes = buf.len();
            ...
            let host_bytes: Vec<u8> = match buf.clone_dtoh(&self.hip_stream) {
                Ok(v) => v,
                Err(_) => continue,
            };
    ...
    fn profile(
        ...
        let peak_bw = crate::rocm_bandwidth_gbps(&self.hip_stream.context());
        let peak_tf = crate::rocm_compute_f32_tflops(&self.hip_stream.context());
                ...
                if let Some(host_op) = llir_graph[host_op_node_index].to_dialect::<dyn HostOp>() {
                    ...
                    node_to_exec.insert(
                        ...
                        exec_graph.add_node(ExecutableHostOp {
                            stream: Arc::clone(&self.hip_stream),
                            inputs,
