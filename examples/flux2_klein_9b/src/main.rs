...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Klein is guidance-distilled — no guidance_scale input.
    println!("Resolution: {width}x{height}, steps={steps}");

    run_full_pipeline(&prompt, width, height, steps)?;
    Ok(())
}
...
// check this function against klein's text encoder
fn run_text_encoder(prompt: &str) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
    ...
    let attention_mask = cx
        ...
        .as_dtype(DType::F32);
    let encoder = text_encoder::Qwen3TextEncoder::init(&mut cx);
    ...
    // HIP context / stream / runtime
    let ctx = HipContext::new(0).unwrap();
    let stream = ctx.default_stream();
    let mut runtime = RocmRuntime::initialize(stream);
...
fn run_full_pipeline(
    ...
    let velocity = model
        .forward(latent_in, text_in, cos_in, sin_in, timestep_in)
        .output();
    ...
    // diffusers passes `timestep=timestep / 1000` (0..1, sigma-like); the
    // transformer rescales by 1000 internally. Klein is distilled, so there is
    // no guidance input.
