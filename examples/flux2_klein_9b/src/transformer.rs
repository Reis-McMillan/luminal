...
pub const HIDDEN: usize = NUM_HEADS * HEAD_DIM; // 4096
pub const MLP_HIDDEN: usize = 12288;
pub const JOINT_ATTENTION_DIM: usize = 12288;
...
impl SingleStreamAttn {
    fn new(prefix: &str, cx: &mut Graph) -> Self {
        let qkv_mlp_out = 3 * HIDDEN + 2 * MLP_HIDDEN; // 12288 + 24576 = 36864
        Self {
            to_qkv_mlp_proj: cx
                .named_tensor(
...
pub struct Flux2Transformer {
    ...
    // Time embedding. Klein is guidance-distilled (`guidance_embeds=false`), so
    // the checkpoint has only a timestep embedder — no guidance embedder.
...
impl Flux2Transformer {
    pub fn init(cx: &mut Graph) -> Self {
        let mk = |name: &str, shape: (usize, usize), cx: &mut Graph| -> GraphTensor {
            cx.named_tensor(name, shape).as_dtype(WEIGHT_DTYPE).persist()
        };
        let mk1 = |name: &str, n: usize, cx: &mut Graph| -> GraphTensor {
            cx.named_tensor(name, n).as_dtype(WEIGHT_DTYPE).persist()
        };
    ...
    /// Compute `temb = timestep_emb`. `timestep` is a `(1,)` F32 graph tensor
    /// set per denoising step. Klein is guidance-distilled, so there is no
    /// guidance embedding term.
    fn embed_time(&self, timestep: GraphTensor) -> GraphTensor {
        // Diffusers' Flux2Transformer2DModel.forward multiplies its timestep
        // input by 1000 before `time_guidance_embed`. The pipeline upstream
        // divides the raw scheduler timestep by 1000 to give the transformer a
        // 0..1 scalar; the transformer multiplies it back to 0..1000 here so
        // the sin/cos `time_proj` argument range matches training. Our
        // `main.rs` feeds the same 0..1 sigma scalar and we mirror the *1000.
        ...
        linear_no_bias(t1, self.time_t2_w)
    }
    ...
    ///   each step.
    /// - `text_embed`: `(S_txt, JOINT_ATTENTION_DIM=12288)`, set once before
    ///   the loop.
    /// - `rope_cos`, `rope_sin`: `(S_txt + S_img, HEAD_DIM=128)`, also set once.
    /// - `timestep`: `(1,)` F32 scalar set per step (0..1 sigma; scaled by 1000
    ///   internally). Klein is distilled — no guidance input.
    ///
    ...
    pub fn forward(
        ...
        let temb = self.embed_time(timestep);
